...
export class OpenAITranslateProvider extends BaseTranslateProvider {
  ...
  protected async doTranslate(queryWordInfo: QueryWordInfo, signal?: AbortSignal): Promise<QueryTypeResult> {
    ...
    // Combine caller signal with network timeout
    ...
    return new Promise((resolve, reject) => {
      fetchSSE(`${url}`, {
        ...
        onDone: () => {
          clearTimeout(timeout);
          let resultText = chunks.join("");

          // Trim trailing right quotes that don't appear in the source text
          const rightQuotes = ['"', "\u201D", "'", "\u300D"];
          if (resultText.length > 0) {
            const lastQueryTextChar = queryWordInfo.word[queryWordInfo.word.length - 1];
            const lastTranslatedTextChar = resultText[resultText.length - 1];
